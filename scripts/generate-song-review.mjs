 ...
 * Generates AI-powered song reviews from Spotify URLs
 */
...
/**
 * Generate review using OpenAI API (Amanda Petrusich style)
 */
...
async function main() {
  ...
  if (!spotifyUrl) {
    ...
    console.error('Usage: node generate-song-review.mjs <spotify-url>');
    process.exit(1);
  }

  try {
    console.log(`🎧 Processing: ${spotifyUrl}\n`);
